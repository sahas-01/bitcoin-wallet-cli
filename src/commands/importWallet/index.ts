...
export default class ImportWallet extends Command {
    static args = {
        mnemonic: Args.string({ description: 'Import a bitcoin wallet by mnemonic', required: true }),
        name: Args.string({ description: 'Name for the new wallet', required: true }),
    };

    static description = 'Import a bitcoin wallet by mnemonic';

    async run(): Promise<void> {
        const { args } = await this.parse(ImportWallet);
        try {
